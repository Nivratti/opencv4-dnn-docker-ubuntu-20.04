...
# change directory for building opencv
build_parent_dir='/usr/local/c++'
mkdir -p ${build_parent_dir}
chmod -R 777 ${build_parent_dir}
cd ${build_parent_dir}


# Configuring tzdata -- timezone
# ------------------
#!/bin/bash
export DEBIAN_FRONTEND=noninteractive

ln -fs /usr/share/zoneinfo/Asia/Calcutta /etc/localtime # or you can set any other timezone
...
cmake -DWITH_QT=OFF -DWITH_OPENGL=ON -DFORCE_VTK=ON -DWITH_TBB=ON -DWITH_GDAL=ON \
      -DWITH_XINE=ON -DENABLE_PRECOMPILED_HEADERS=OFF ..
fi

if [ $OPENCV_CONTRIB = 'YES' ]; then
cmake -DWITH_QT=OFF -DWITH_OPENGL=ON -DFORCE_VTK=ON -DWITH_TBB=ON -DWITH_GDAL=ON \
      -DWITH_XINE=OFF -DENABLE_PRECOMPILED_HEADERS=OFF \
      -DOPENCV_EXTRA_MODULES_PATH=../opencv_contrib/modules ..
fi

# make
# make -j8  # parallel 8 core
make -j$(nproc)
make install
ldconfig
